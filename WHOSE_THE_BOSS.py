...
db1.commit()
c1.execute("create table if not exists stock(pName varchar(30) primary key,pid varchar(20),Quantity int(3),brand varchar(20),Price int(4))")
c1.execute("create table bill(oid varchar(30) primary key, CustomerName varchar(20),PhoneNumber char(10) unique key,pname varchar(30),Quantity int(100),Price int(4),foreign key (pname) references stock(pname))")
db1.commit()
...
def add_stock():
    ...
    if row is not None:
        ...
        c1.execute("insert into Stock(pname,pid,quantity,brand,price) values('"+prod+"','"+p_id+"','"+str(quantity)+"','"+br+"','"+str(price)+"')")
        db1.commit()
        print("""++++++++++++++++++++++++SUCCESSFULLY ADDED++++++++++++++++++++++++""")
...
def billing():
    ...
    if max(lk)<n:
        ...
        log=c1.fetchone()
        if log is not None:
            c1.execute("insert into Record values('"+cusname+"','"+str(phno)+"','"+prod+"','"+str(n)+"','"+str(price)+"')")
...
def staff():
    ...
    if ch==1:
        ...
        c1.execute("insert into Users(name,gender,age,phonenumber,address) values('"+fname+"','"+gender+"','"+str(age)+"','"+str(phno)+"','"+add+"')")
        print("""++++++++++++++++++++++++++++++STAFF IS SUCCESSFULLY ADDED++++++++++++++++++++++++++++++""")
        db1.commit()
     
    elif ch==2:
        nm=str(input("Enter staff name to remove:"))
        c1.execute("select name from users where name='"+nm+"'")
        toy=c1.fetchone()
        if toy is not None:
            c1.execute("delete from users where name='"+nm+"'")
...
def sell_r():
    ...
    if ty==1:
        c1.execute("select * from record")
        for u in c1:
            print(u)
    if ty==2:
        bb=input("Are you sure(Y/N):")
        if bb=="Y":
            c1.execute("delete from record")
            db1.commit()
        elif bb=="N":
            pass
...
if login():
    while True:
        ...
        print("Press 5 - Show Sell record")
